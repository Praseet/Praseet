![snake](https://raw.githubusercontent.com/Praseet/Praseet/output/snake.svg)

# Hi there, I'm Praseet Shukla 👋

```javascript
const praseet = {
  pronouns: "He" | "Him",
  code: ["Python", "TypeScript", "HTML", "CSS", "JavaScript"],
  askMeAbout: ["web development", "coding", "AI", "quantum computing"],
  technologies: {
    frontEnd: {
      js: ["React", "Redux", "Angular"],
      css: ["Bootstrap", "Material-UI"]
    },
    backEnd: {
      python: ["Django", "Flask"],
      js: ["Node", "Express"]
    },
    databases: ["MongoDB", "MySQL", "SQLite"],
  },
  architecture: ["Microservices", "RESTful APIs", "MVC"],
  funFact: "Stay hungry, stay foolish - Steve Jobs"
};
```

## GitHub Stats
[![Anurag's GitHub stats](https://github-stats-extended.vercel.app/api?username=Praseet)](https://github.com/stats-organization/github-stats-extended)

## Top Languages
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Praseet&layout=compact)

## Trophy Case
[![trophy](https://github-profile-trophy.vercel.app/?username=Praseet)](https://github.com/ryo-ma/github-profile-trophy)

## GitHub Streak
[![GitHub Streak](https://streak-stats.demolab.com/?user=Praseet)](https://git.io/streak-stats)

## Skills & Interests
- 🌐 Web Development Enthusiast
- 🤖 AI and Quantum Computing Advocate

#

If you resonate with my work, feel free to reach out to me for collaboration or just to chat about emerging technologies!

Let's connect:
- LinkedIn: [Praseet Shukla](https://www.linkedin.com/in/praseet-shukla-88563b376/)
- Email: praseetshukla1025@gmail.com

Thanks for stopping by and happy coding! 🚀🎉

![Visitor Count](https://profile-counter.glitch.me/Praseet/count.svg)



